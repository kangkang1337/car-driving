...
#define SG90_SLOW_STEP_US 40
#define SG90_SETTLE_PULSES 6
...
static unsigned int g_servoDutyUs = SG90_CENTER_DUTY_US;
...
static void ServoMoveTo(unsigned int targetDutyUs)
{
    while (g_servoDutyUs != targetDutyUs) {
        if (g_servoDutyUs < targetDutyUs) {
            unsigned int nextDuty = g_servoDutyUs + SG90_SLOW_STEP_US;
            g_servoDutyUs = (nextDuty > targetDutyUs) ? targetDutyUs : nextDuty;
        } else {
            unsigned int nextDuty = g_servoDutyUs - SG90_SLOW_STEP_US;
            g_servoDutyUs = (nextDuty < targetDutyUs) ? targetDutyUs : nextDuty;
        }
        ServoPulse(g_servoDutyUs);
    }

    for (int i = 0; i < SG90_SETTLE_PULSES; i++) {
        ServoPulse(targetDutyUs);
    }
}
...
static void UltrasonicGpioInit(void)
{
    ...
    ServoMoveTo(SG90_LEFT_DUTY_US);
    ServoMoveTo(SG90_CENTER_DUTY_US);
    ServoMoveTo(SG90_RIGHT_DUTY_US);
    ServoMoveTo(SG90_CENTER_DUTY_US);
}
...
static void AvoidObstacle(void)
{
    ...
    CarBackward();
    DelayMs(OBSTACLE_BACKWARD_TIME_MS);
    CarStop();
    DelayMs(60);
    ...
    while (1) {
        ...
        switch (state) {
            ...
            case CAR_STATE_EDGE_TURN:
                elapsedMs += SAMPLE_PERIOD_MS;
                if (IsTableSafe(&ir)) {
                    safeElapsedMs += SAMPLE_PERIOD_MS;
                } else {
                    safeElapsedMs = 0;
                }
                if (elapsedMs >= EDGE_TURN_TIME_MS && safeElapsedMs >= EDGE_RELEASE_TIME_MS) {
...
static void HuaweiCloudTask(void *arg)
{
    ...
    while (1) {
        ...
        DelayMs(CLOUD_UPLOAD_PERIOD_MS);
    }
}
